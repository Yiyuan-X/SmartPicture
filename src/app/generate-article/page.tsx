          ...
          <div className="grid gap-6 lg:grid-cols-[1.1fr,0.9fr]">
            ...
            <div className="space-y-6">
                  ...
                  {article.shareSnippets.map((snippet, index) => (
                    <button
                      ...
                      onClick={async () => {
                        ...
                        if (ok) {
                          if (snippet.platform === "facebook") {
                            addPoints(SHARE_REWARD);
                            setSuccessMessage(
                              `Facebook 文案已复制，分享即可获赠 ${SHARE_REWARD} 积分！`
                            );
                          } else {
                            setSuccessMessage(
                              `${shareLabels[snippet.platform] ?? snippet.platform} 文案已复制，欢迎同步到该平台。`
                            );
                          }
                        }
                      }}
                    >
                      <span className="block text-xs font-semibold text-orange-600">
                        {shareLabels[snippet.platform] ?? snippet.platform.toUpperCase()}
                      </span>
                      <span className="block whitespace-pre-wrap">{snippet.message}</span>
                    </button>
                  ))}
                </CardContent>
              </Card>
              ...
              <Card className="border-blue-100 bg-white/90 shadow-sm">
                <CardHeader>
                  <CardTitle className="text-lg">文章分发清单</CardTitle>
                  <CardDescription>复制下列任务可导入到你的内容日历</CardDescription>
                </CardHeader>
                <CardContent className="space-y-2 text-sm text-gray-600">
                  <ul className="space-y-1 list-disc pl-5">
                    <li>发布至 SmartPicture 官方博客，并附上 SmartPicture 工具 CTA</li>
                    <li>同步至企业微信社群和内部知识库</li>
                    <li>制作 30 秒短视频摘要，在抖音/快手发布</li>
                    <li>邀请好友体验该生文功能，双方各得 30 积分（示例）</li>
                  </ul>
                </CardContent>
              </Card>
            </div>
        ...
        <div className="fixed inset-0 z-[9998] flex items-center justify-center bg-black/40 px-4">
          <div className="w-full max-w-md rounded-2xl bg-white p-6 text-center shadow-2xl">
            ...
            <p className="mt-2 text-sm text-gray-600">
              登录 SmartPicture 账号即可解锁 AI 自动写作、分享赚积分等功能。
            </p>
